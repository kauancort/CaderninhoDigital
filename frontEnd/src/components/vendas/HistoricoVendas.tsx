...
export function HistoricoVendas() {
  ...
  const paginaQuery = useQuery({
    ...
    queryFn: () => listarHistoricoVendas({ ...comuns, pagina, tamanho: 15 }),
      ...
      >
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Select
            ...
            options={[
              { value: "dataVenda", label: "Data" },
              { value: "valorTotal", label: "Valor" },
              { value: "cliente", label: "Cliente" },
              { value: "statusPagamento", label: "Status" },
            ]}
          />
          <Select
            label="Direção"
            ...
            options={[
              { value: "DESC", label: "Mais recentes/maiores" },
              { value: "ASC", label: "Mais antigos/menores" },
            ]}
          />
        </div>
      </FiltrosRecolhiveis>
                ...
                {dados.registros.map((venda) => (
                  <tr key={venda.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3">
                      <strong className="block text-foreground">Venda #{venda.id}</strong>
...
function ResumoHistorico({ carregando, dados }: { carregando: boolean; dados?: any }) {
  const cards = [
    { label: "Faturamento no período", value: fmtBRL(dados?.faturamento ?? 0) },
    { label: "Quantidade de vendas", value: String(dados?.quantidadeVendas ?? 0) },
    { label: "Itens vendidos", value: String(Number(dados?.quantidadeItens ?? 0)) },
    { label: "Ticket médio", value: fmtBRL(dados?.ticketMedio ?? 0) },
  ];
  return (
...
export function StatusBadge({ status, atraso }: { status: string; atraso?: boolean }) {
  const label = atraso
    ? "Em atraso"
    : ({ PAGO: "Pago", PENDENTE: "Pendente", ATRASADO: "Atrasado", NAO_SE_APLICA: "N/A" }[status] ??
      status);
  const classe =
    status === "PAGO"
...
function Paginacao(props: {
  ...
  if (props.paginas <= 1) return null;
  return (
