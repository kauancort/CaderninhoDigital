...
import { CircleDollarSign, Plus, ReceiptText, Truck } from "lucide-react";
...
export function VendasLayout({
  ...
  const emTransporte = location.pathname === "/vendas/transporte";
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground md:text-base">
            Acompanhe o histórico comercial, transportes e recebimentos.
          </p>
        </div>
      ...
      <nav
        className="grid grid-cols-3 rounded-2xl border border-border bg-card p-1.5 shadow-warm-sm"
        role="tablist"
      ...
      >
        <Link
          to="/vendas"
          role="tab"
          aria-selected={!emReceber && !emTransporte}
          className={[
        ...
        <Link
          to="/vendas/transporte"
          role="tab"
          aria-selected={emTransporte}
          className={[
            "inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-3 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
            emTransporte
              ? "bg-primary text-primary-foreground shadow-warm-sm"
