...
import { Check, MapPin, UserRound, FileText, X } from "lucide-react";
import { clienteSchema } from "@/lib/clientes.functions";
import {
  mascararDocumento,
  somenteDigitos,
  validarCnpj,
...
import { clienteFormVazio, UFS_BRASIL, type ClienteFormData } from "@/lib/cliente-form";
...
export function ClienteFormModal({
  ...
  const [tipo, setTipo] = useState<TipoDocumento>(
    somenteDigitos(inicial.documento).length > 11 ? "CNPJ" : "CPF",
  );
  ...
  const ultimoCep = useRef(apenasDigitosCep(inicial.cep) || null);
  const ultimoAutomatico = useRef({ endereco: "", bairro: "", cidade: "", estado: "" });
  const envioTravado = useRef(false);

  useEffect(() => {
    if (!aberto) return;
    setForm(inicial);
    setTipo(somenteDigitos(inicial.documento).length > 11 ? "CNPJ" : "CPF");
    setErros({});
    setStatusCep(null);
    ultimoCep.current = apenasDigitosCep(inicial.cep) || null;
  }, [aberto, inicial]);

  useEffect(() => () => consultaRef.current?.abort(), []);
  useEffect(() => {
    if (!salvando) envioTravado.current = false;
  }, [salvando]);
  if (!aberto) return null;

  const alterar = (campo: keyof ClienteFormData, valor: string) => {
  ...
  async function buscarCep() {
    ...
    if (!cep || cep.length !== 8 || cep === ultimoCep.current) return;
    ...
    try {
      const encontrado = await consultarCep(cep, controller.signal);
      ...
      setForm((atual) => {
        if (apenasDigitosCep(atual.cep) !== cep) return atual;
        const podePreencher = (campo: "endereco" | "bairro" | "cidade" | "estado") =>
          !atual[campo] || atual[campo] === ultimoAutomatico.current[campo];
        const proximo = {
          ...atual,
          endereco: podePreencher("endereco") ? encontrado.endereco : atual.endereco,
          bairro: podePreencher("bairro") ? encontrado.bairro : atual.bairro,
          cidade: podePreencher("cidade") ? encontrado.cidade : atual.cidade,
          estado: podePreencher("estado") ? encontrado.estado : atual.estado,
        };
        ultimoAutomatico.current = encontrado;
        return proximo;
      });
      setStatusCep("Endereço encontrado. Confira os dados abaixo.");
      numeroRef.current?.focus();
    } catch {
      if (!controller.signal.aborted)
        setStatusCep("Não foi possível consultar o CEP. Preencha o endereço manualmente.");
    }
  }

  async function enviar(event: React.FormEvent) {
    event.preventDefault();
    if (salvando || envioTravado.current) return;
    const normalizado = {
      ...form,
      cep: apenasDigitosCep(form.cep),
      documento: somenteDigitos(form.documento),
    };
    const resultado = clienteSchema.safeParse(normalizado);
    ...
    if (form.cep && apenasDigitosCep(form.cep).length !== 8)
      novosErros.cep = "Informe um CEP válido com 8 dígitos.";
    if (tipo === "CPF" && !validarCpf(form.documento))
      novosErros.documento = "O CPF informado não é válido.";
    if (tipo === "CNPJ" && !validarCnpj(form.documento))
      novosErros.documento = "O CNPJ informado não é válido.";
    setErros(novosErros);
    const primeiro = Object.keys(novosErros)[0];
    if (primeiro) {
      formRef.current?.querySelector<HTMLElement>(`[data-field="${primeiro}"]`)?.focus();
      return;
    }
  ...
  return (
    <div
      ...
      onMouseDown={(e) => e.target === e.currentTarget && !salvando && onClose()}
    >
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card px-5 py-4 md:px-8">
          <div>
            <h2 id="cliente-modal-title" className="font-display text-2xl font-bold text-primary">
              {titulo}
            </h2>
        ...
        >
          <div className="min-h-0 flex-1 space-y-7 overflow-y-auto bg-background/40 px-5 py-6 md:px-8">
            <Secao icon={<UserRound size={20} />} titulo="Dados principais">
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Campo campo="telefone" label="Telefone" erro={erros.telefone}>
                  <input
                    data-field="telefone"
                    type="tel"
                    inputMode="numeric"
                ...
                <Campo campo="email" label="E-mail (opcional)" erro={erros.email}>
                  <input
                    data-field="email"
                    type="email"
                    value={form.email}
            ...
            <Secao icon={<FileText size={20} />} titulo="Documento">
              <fieldset>
                <legend className="mb-3 text-base font-semibold text-foreground">
                  Tipo de cliente
                </legend>
                <div className="grid grid-cols-2 gap-3">
                  {(["CPF", "CNPJ"] as const).map((item) => (
                    <button
                      key={item}
                      type="button"
                      ...
                      className={`min-h-12 rounded-xl border-2 px-3 text-base font-bold ${tipo === item ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:bg-secondary"}`}
                    >
                      {item === "CPF" ? "Pessoa física — CPF" : "Pessoa jurídica — CNPJ"}
                    </button>
                  ))}
                </div>
              </fieldset>
              <div className={`mt-5 grid gap-5 ${tipo === "CNPJ" ? "md:grid-cols-2" : ""}`}>
                <Campo campo="documento" label={tipo} erro={erros.documento}>
                  <input
                    data-field="documento"
                    inputMode="numeric"
                    value={form.documento}
                    onChange={(e) => alterar("documento", mascararDocumento(e.target.value, tipo))}
                    className="ds-input min-h-12 text-base"
            ...
            <Secao icon={<MapPin size={20} />} titulo="Endereço">
              <div className="grid gap-5 md:grid-cols-6">
                ...
                >
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (apenasDigitosCep(cep) !== ultimoCep.current) consultaRef.current?.abort();
                      alterar("cep", cep);
                      setStatusCep(null);
                      if (apenasDigitosCep(cep).length !== 8) ultimoCep.current = null;
                    }}
                    onBlur={buscarCep}
                    className="ds-input min-h-12 text-base"
                ...
                <Campo campo="numero" label="Número" erro={erros.numero} className="md:col-span-1">
                  <input
                    ref={numeroRef}
                    data-field="numero"
                    value={form.numero}
                ...
                <Campo campo="bairro" label="Bairro" erro={erros.bairro} className="md:col-span-3">
                  <input
                    data-field="bairro"
                    value={form.bairro}
                ...
                <Campo campo="cidade" label="Cidade" erro={erros.cidade} className="md:col-span-4">
                  <input
                    data-field="cidade"
                    value={form.cidade}
                ...
                <Campo campo="estado" label="Estado" erro={erros.estado} className="md:col-span-2">
                  <select
                    data-field="estado"
                    value={form.estado}
...
function Campo({
  ...
  return (
    <div className={className}>
      <label className="mb-2 block text-base font-bold text-foreground">{label}</label>
