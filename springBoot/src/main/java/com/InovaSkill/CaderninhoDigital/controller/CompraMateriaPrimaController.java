...
public class CompraMateriaPrimaController {
    ...
    @PostMapping
    public ResponseEntity<CompraMateriaPrimaResponseDTO> criar(@RequestHeader("X-Usuario-Id") Long usuarioId, @RequestBody @Valid CompraMateriaPrimaRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(compraService.criar(usuarioId, dto));
    }

    @GetMapping
    public ResponseEntity<List<CompraMateriaPrimaResponseDTO>> listar(@RequestHeader("X-Usuario-Id") Long usuarioId) {
        return ResponseEntity.ok(compraService.listar(usuarioId));
    }

    @GetMapping("/{id}")
    public ResponseEntity<CompraMateriaPrimaResponseDTO> buscar(@RequestHeader("X-Usuario-Id") Long usuarioId, @PathVariable Long id) {
