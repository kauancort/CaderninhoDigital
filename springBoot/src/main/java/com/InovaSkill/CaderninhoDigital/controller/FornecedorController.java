...
public class FornecedorController {
    ...
    @PostMapping
    public ResponseEntity<FornecedorResponseDTO> criar(@RequestHeader("X-Usuario-Id") Long usuarioId, @RequestBody @Valid FornecedorRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(fornecedorService.criar(usuarioId, dto));
    }

    @GetMapping
    public ResponseEntity<List<FornecedorResponseDTO>> listar(@RequestHeader("X-Usuario-Id") Long usuarioId) {
        return ResponseEntity.ok(fornecedorService.listar(usuarioId));
    }

    @GetMapping("/{id}")
    public ResponseEntity<FornecedorResponseDTO> buscar(@RequestHeader("X-Usuario-Id") Long usuarioId, @PathVariable Long id) {
        return ResponseEntity.ok(fornecedorService.buscar(usuarioId, id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<FornecedorResponseDTO> atualizar(@RequestHeader("X-Usuario-Id") Long usuarioId, @PathVariable Long id, @RequestBody @Valid FornecedorRequestDTO dto) {
